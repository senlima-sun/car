...
function toCanvas(x: number, y: number): [number, number] {
  return [
    PADDING + (x + 0.5) * SCALE,
    CANVAS_SIZE - PADDING - (y + 0.5) * SCALE,
  ]
}
...
function toWorld(cx: number, cy: number): [number, number] {
  return [
    ...
    (CANVAS_SIZE - PADDING - cy) / SCALE - 0.5,
  ]
}
...
export default function ShapePointsEditor({
  ...
  const findAlignment = useCallback((x: number, y: number, excludeIndex: number): { x?: number; y?: number; snappedX: number; snappedY: number } => {
    ...
    for (let i = 0; i < allPts.length; i++) {
      ...
      if (typeof editMode === 'number') {
        const holeStartIdx = points.length + holes.slice(0, editMode).reduce((sum, h) => sum + h.length, 0)
  ...
  const applySnap = useCallback((x: number, y: number, excludeIndex: number = -1): [number, number] => {
    if (!snapEnabled) {
      return [
    ...
    // Clamp to bounds
    return [
      Math.max(-0.5, Math.min(1.5, snappedX)),
      Math.max(-0.5, Math.min(1.5, snappedY))
    ]
  }, [snapEnabled, findAlignment])

  // Update points based on edit mode
  const updateCurrentPoints = useCallback((newPoints: [number, number][]) => {
    if (editMode === 'outline') {
      onChange(newPoints)
  ...
  }, [points, holes, currentPoints, hoveredIndex, draggingIndex, editMode, snapEnabled, alignGuides])
  ...
  const handleReset = () => {
    if (editMode === 'outline') {
      ...
      onChange([[0, 0], [1, 0], [1, 0.6], [0.5, 1], [0, 0.6]])
    } else {
      // Reset hole to small rectangle
      updateCurrentPoints([[0.3, 0.2], [0.7, 0.2], [0.7, 0.4], [0.3, 0.4]])
    }
  }
  ...
  const handleAddHole = () => {
    ...
    const newHole: [number, number][] = [[0.3, 0.2], [0.7, 0.2], [0.7, 0.4], [0.3, 0.4]]
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '8px', fontSize: '10px', color: '#888' }}>
        {editMode === 'outline' ? 'Editing outer shape' : `Editing hole ${(editMode as number) + 1}`}
      ...
      <div style={{ marginTop: '8px', display: 'flex', gap: '6px', alignItems: 'center' }}>
        ...
        {typeof editMode === 'number' && onHolesChange && (
          <button
            style={{ ...buttonStyle, background: '#703a3a' }}
