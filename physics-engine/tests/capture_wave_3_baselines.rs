...
/// Wave 3 final-baseline capture (Phase 7 Step 7.1). Writes all eight
/// scenarios to `wave_3_baselines.json` — the new strict ±0.5% gate
/// fixture. The Phase 0 dry/wet anchors stay frozen as pre-Wave-3
/// records.
#[test]
#[ignore]
fn capture_wave_3_final_baselines() {
  ...
  "notes": "Captured at Wave 3 head (post Phase 0-6). Promoted to the strict ±0.5% calibration gate at Phase 7. Replaces the Phase 0 wave_3_dry_baselines.json / wave_3_wet_baselines.json snapshots which were the pre-drift anchors.",
  "scenarios": {{
...
"#,
    ...
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/wave_3_baselines.json");
    std::fs::create_dir_all(path.parent().unwrap()).unwrap();
    std::fs::write(&path, json).expect("write Wave 3 baselines");
