...
#[test]
#[ignore]
fn capture_step_and_sync_perf_wave_4_pre() {
    let mut engine = PhysicsEngine::new();
    engine.set_surface(SurfaceType::Road);
  ...
  "notes": "Pre-Wave-4 perf anchor. Phase 7 wave-end snapshot must stay within 1.05× this p50."
}}
"#,
        SAMPLE_COUNT, min, p50, p99
    );
    ...
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/perf_wave_4_pre.json");
    std::fs::write(&path, json).expect("write Wave 4 pre-perf fixture");
